...
class UnifiedWindowDataset(Dataset):
    def __init__(self, root_dir: str, transform: transforms.Compose, subset_size: int = None) -> None:
        ...
        expected_classes = ["normal", "damaged-rear-window", "damaged-window", "damaged-windscreen"]
        
        if len(self.class_names) != 4 or set(self.class_names) != set(expected_classes):
...
def build_dataloaders(cfg: TrainConfig) -> Tuple[DataLoader, DataLoader, Dict[str, int]]:
    ...
    full_dataset = UnifiedWindowDataset(train_dir, train_tf, subset_size=cfg.subset_size)
    ...
    val_dataset_full = UnifiedWindowDataset(train_dir, val_tf, subset_size=cfg.subset_size)
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="datasets/unified_windows")
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(cfg.output_dir, "unified_windows.pt")
            torch.save({
    ...
    history_path = os.path.join(cfg.output_dir, "unified_windows_history.json")
