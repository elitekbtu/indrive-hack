...
class CSVImageDataset(Dataset):
    def __init__(self, root_dir: str, transform: transforms.Compose, subset_size: int = None) -> None:
        ...
        if len(self.class_names) != 3:
            raise RuntimeError(f"Expected exactly 3 class columns, got {len(self.class_names)}")
...
def build_dataloaders(cfg: TrainConfig) -> Tuple[DataLoader, DataLoader, Dict[str, int]]:
    ...
    full_dataset = CSVImageDataset(train_dir, train_tf, subset_size=cfg.subset_size)
    ...
    val_dataset_full = CSVImageDataset(train_dir, val_tf, subset_size=cfg.subset_size)
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="datasets/damaged_windows")
    ...
    parser.add_argument("--image_size", type=int, default=224)  # Smaller for faster training
    parser.add_argument("--arch", type=str, default="resnet18", choices=["resnet18", "mobilenet", "simple"])
    parser.add_argument("--subset_size", type=int, default=300, help="Use subset of data for faster training")
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(cfg.output_dir, "damaged_windows.pt")
            torch.save({
    ...
    history_path = os.path.join(cfg.output_dir, "damaged_windows_history.json")
