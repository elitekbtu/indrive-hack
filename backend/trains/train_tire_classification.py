...
def build_transforms(image_size: int) -> Tuple[transforms.Compose, transforms.Compose]:
    # Transforms for tire classification - improved augmentation for better generalization
...
class TireClassificationDataset(Dataset):
    def __init__(self, root_dir: str, transform: transforms.Compose, subset_size: int = None) -> None:
        ...
        expected_classes = ["Flat-tire", "Full-tire"]
...
def build_dataloaders(cfg: TrainConfig) -> Tuple[DataLoader, DataLoader, Dict[str, int]]:
    ...
    full_dataset = TireClassificationDataset(train_dir, train_tf, subset_size=cfg.subset_size)
    ...
    val_dataset_full = TireClassificationDataset(train_dir, val_tf, subset_size=cfg.subset_size)
...
def main():
    ...
    parser.add_argument("--data_root", type=str, default="datasets/full vs flat tire.v1i.multiclass")
    ...
    parser.add_argument("--subset_size", type=int, default=800, help="Use subset of data for faster training")
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(cfg.output_dir, "tire_classification.pt")
            torch.save({
    ...
    history_path = os.path.join(cfg.output_dir, "tire_classification_history.json")
