...
@torch.no_grad()
def predict_image_bytes(model: nn.Module, tf: transforms.Compose, image_bytes: bytes, device: Optional[str] = None) -> Dict:
    if device is None:
    ...
    return {
        "pred_idx": pred_idx,
        "pred_label": model.idx_to_class[pred_idx] if hasattr(model, "idx_to_class") else str(pred_idx),
